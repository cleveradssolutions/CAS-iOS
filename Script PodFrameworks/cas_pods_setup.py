...
O=''
L='store_true'
import sys as Q,os as E,subprocess as f,shutil as M,argparse as A1,json
g='3.5.2'
r='13.0'
G='CASFrameworks'
s=G+'.xcworkspace'
...
Y=U()
Z=U()
F=A1.ArgumentParser(prog='python cas_pods_setup.py',description='This script is designed to seamlessly load and build CAS Adapters, incorporating essential libraries from Cocoapods into the Frameworks directory to facilitate the iOS build process.',epilog='Powered by CAS.AI version '+A2)
F.add_argument('mode',default=v,choices=[v,w],help='[setup] - Download CAS adapters and compile any sources from Cocoapods and place to Frameworks directory.\n'+"[podfile] - Create a Podfile in the same directory. Open the generated Podile in any text editor to manually add any pod names to the target '"+G+"'.")
...
F.add_argument(x,action=L,help='Skip running `pod repo update` before install.')
...
def I(str):D('\n\x1b[95m'+str+n)
def t(str):print('\x1b[91m   '+str+n)
def a(str):print('\x1b[96m   '+str+n)
def S(error):t(error);Q.exit()
...
def A5():
	...
	if not K(W)or not K(i)or not K(A(C,o)):
		E=['pod update']
		if B.no_repo_update:E.append(x)
		for F in j(E):
			if not B.silent:
...
def A6():
	...
	T(i);E.remove(W)
def A7():
	I('## Pods building ... (this may take a while)');T(p);E=['xcodebuild',p,'-workspace',s,'-scheme',G,'-derivedDataPath',"'"+C+"/build/'",'-sdk','iphoneos','-destination',"'generic/platform=iOS'",'-configuration','Release','IPHONEOS_DEPLOYMENT_TARGET='+r,'GCC_GENERATE_DEBUGGING_SYMBOLS=NO']
	for A in j(E):
		if' error:'in A:S(A)
		if' error 'in A:t(A)
...
def A8():
	...
	for F in k(N,h):
		if not F.endswith(G+N):
			K=E.path.splitext(E.path.basename(F))[0];D(y+K+P)
...
def AA():
	I('## Find Resources');L=A(C,o);O=A(L,z,e+G,e+G+'-resources-Release-input-files.xcfilelist')
	with open(O)as N:
		next(N)
		for P in N:
			...
			if B.embed_archive:Q=u(F);K=A(C,Q+d,F)
			else:K=A(J,F)
			M.move(H,K);Z.add(K)
def u(bundleName):
	A=bundleName
...
def AB():
	...
	for B in H:B+=d;D('   '+B+F);G=M.make_archive(base_name=B,format='zip',root_dir=C,base_dir=B);M.move(G,A(J,B+F));T(B)
	D('\n - Installed '+V(len(H))+' frameworks')
def AC():
	F=A(C,o,z,e+G,e+G+'.release.xcconfig')
	with open(F)as I:
		for D in I:
...
def AF():
	E='.xcodeproj'
	if B.mode==w:l(B.adapters);D(O);D(F.epilog);exit()
	if not K(A(C,G+E)):S('The script should be in the same directory as the '+G+E)
	if not K(A(C,A0)):l(B.adapters)
