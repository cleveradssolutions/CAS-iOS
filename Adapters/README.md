## CAS.AI Mediation adapters

The adapters in the table are listed in alphabetical order.

| Name | Version | Supported Formats | SPM | Optimal | Families | VPN | Changelog | SDK |
| ---- | :-----: | :---------------: | :-: | :-----: | :------: | :-: | :-------: | :-: |
| AppLovin | 13.6.3.2-alpha49 | Banner, Interstitial, Rewarded, AppOpen, Native | **✓** | **✓** | **✗** | **✗** | [view](AppLovin/CHANGELOG.md) | [link](https://github.com/AppLovin/AppLovin-MAX-SDK-iOS/releases) |
| AudienceNetwork | 6.22.0.0 | Banner, Interstitial, Rewarded, Native | **✓** | **✓** | **✗** | **✗** | [view](AudienceNetwork/CHANGELOG.md) | [link](https://developers.facebook.com/docs/audience-network/setting-up/platform-setup/ios/changelog?locale=en_US) |
| Bidease 🧪 | 2.2.7.0 | Banner, Interstitial, Rewarded | **✗** | **✗** | **✗** | **✗** | [view](Bidease/CHANGELOG.md) | [link](https://devs.bidease.com/docs/changelogs/ios) |
| Bigo | 5.3.0.0 | Banner, Interstitial, Rewarded, AppOpen, Native | **✗** | **✓** | **✗** | **✓** | [view](Bigo/CHANGELOG.md) | [link](https://www.bigossp.com/guide/sdk/ios/version) |
| CASExchange | 4.6.3.0 | Banner, Interstitial, Rewarded, Native | **✓** | **✓** | **✗** | **✗** | [view](CASExchange/CHANGELOG.md) | [link]() |
| Chartboost | 9.13.0.0 | Banner, Interstitial, Rewarded | **✓** | **✗** | **✓** | **✗** | [view](Chartboost/CHANGELOG.md) | [link](https://docs.chartboost.com/en/monetization/integrate/ios/changelog/) |
| CrossPromo | 4.8.0.0 | Banner, Interstitial, Rewarded | **✓** | **✗** | **✗** | **✗** | [view](CrossPromo/CHANGELOG.md) | [link]() |
| DTExchange | 8.4.9.0 | Banner, Interstitial, Rewarded | **✓** | **✗** | **✗** | **✗** | [view](DTExchange/CHANGELOG.md) | [link](https://developer.digitalturbine.com/hc/en-us/articles/360010922578-DT-Exchange-iOS-Changelog) |
| DisplayIO 🧪 | 4.7.5.0 | Banner, Interstitial, Rewarded | **✓** | **✗** | **✗** | **✗** | [view](DisplayIO/CHANGELOG.md) | [link](https://www.display.io/documentation/ios-changelog/) |
| GoogleAds | 13.7.0.0-alpha49 | Banner, Interstitial, Rewarded, AppOpen, Native | **✓** | **✓** | **✓** | **✓** | [view](GoogleAds/CHANGELOG.md) | [link](https://developers.google.com/admob/ios/rel-notes?hl=en) |
| HyprMX | 6.4.6.0 | Banner, Interstitial, Rewarded | **✓** | **✗** | **✗** | **✗** | [view](HyprMX/CHANGELOG.md) | [link](https://documentation.hyprmx.com/ios-hyprmx-sdk/downloads-change-log/change-log/ios-sdk-change-log) |
| InMobi | 11.4.0.0 | Banner, Interstitial, Rewarded, Native | **✓** | **✓** | **✓** | **✓** | [view](InMobi/CHANGELOG.md) | [link](https://support.inmobi.com/monetize/sdk-documentation/ios-guidelines/changelogs) |
| IronSource | 9.5.0.1-alpha49 | Banner, Interstitial, Rewarded | **✓** | **✓** | **✓** | **✓** | [view](IronSource/CHANGELOG.md) | [link](https://docs.unity.com/en-us/grow/levelplay/sdk/ios/changelog) |
| Kidoz | 10.1.5.1 | Banner, Interstitial, Rewarded | **✓** | **✗** | **✓** | **✗** | [view](Kidoz/CHANGELOG.md) | [link](https://github.com/Kidoz-SDK/kidoz-mobile-sdk/tree/main/Direct/iOS) |
| LiftoffMonetize | 7.7.6.1 | Banner, Interstitial, Rewarded, AppOpen, Native | **✓** | **✓** | **✓** | **✓** | [view](LiftoffMonetize/CHANGELOG.md) | [link](https://support.vungle.com/hc/en-us/articles/15718672681883) |
| Maticoo | 2.2.0.0 | Banner, Interstitial, Rewarded, Native | **✗** | **✓** | **✗** | **✗** | [view](Maticoo/CHANGELOG.md) | [link](https://doc.zmaticoo.com/#/help/en/1802/) |
| Mintegral | 8.1.6.0 | Banner, Interstitial, Rewarded, AppOpen, Native | **✓** | **✓** | **✓** | **✓** | [view](Mintegral/CHANGELOG.md) | [link](https://dev.mintegral.com/doc/index.html?file=sdk-m_sdk-ios&lang=en) |
| Moloco 🧪 | 4.9.0.0 | Banner, Interstitial, Rewarded | **✓** | **✗** | **✗** | **✗** | [view](Moloco/CHANGELOG.md) | [link](https://moloco.github.io/ios-sdk-docs/) |
| Monetrix 🧪 | 1.2.0.5 | Banner, Interstitial, Rewarded | **✗** | **✗** | **✗** | **✗** | [view](Monetrix/CHANGELOG.md) | [link]() |
| MonetrixVPN 🧪 | 1.2.0.5 | Banner, Interstitial, Rewarded | **✗** | **✗** | **✗** | **✓** | [view](MonetrixVPN/CHANGELOG.md) | [link]() |
| Ogury | 5.2.3.0 | Banner, Interstitial, Rewarded | **✓** | **✗** | **✗** | **✗** | [view](Ogury/CHANGELOG.md) | [link](https://support.ogury.com/inapp/release-notes/ogury-sdk/ios) |
| Pangle | 8.2.0.8.0 | Banner, Interstitial, Rewarded, AppOpen, Native | **✓** | **✓** | **✗** | **✗** | [view](Pangle/CHANGELOG.md) | [link](https://github.com/bytedance/Bytedance-UnionAD) |
| Prado | 10.1.5.1 | Banner, Interstitial, Rewarded | **✓** | **✗** | **✗** | **✗** | [view](Prado/CHANGELOG.md) | [link](https://github.com/Prado-SDK/prado-mobile-sdk/tree/main/Direct/iOS) |
| PubMatic 🧪 | 5.2.0.1 | Banner, Interstitial, Rewarded | **✓** | **✗** | **✗** | **✗** | [view](PubMatic/CHANGELOG.md) | [link](https://help.pubmatic.com/openwrap/reference/changelog-5) |
| Smaato | 23.2.0.1 | Banner, Interstitial, Rewarded | **✓** | **✗** | **✗** | **✗** | [view](Smaato/CHANGELOG.md) | [link](https://developers.smaato.com/publishers/nextgen-sdk-ios-changelog/) |
| StartIO | 4.14.0.0 | Banner, Interstitial, Rewarded, AppOpen, Native | **✓** | **✗** | **✗** | **✗** | [view](StartIO/CHANGELOG.md) | [link](https://support.start.io/hc/en-us/articles/6827061324434-iOS-SDK-Change-Log) |
| SuperAwesome | 9.4.0.1 | Interstitial, Rewarded | **✗** | **✗** | **✗** | **✗** | [view](SuperAwesome/CHANGELOG.md) | [link](https://sdks.superawesome.com/docs/ios/releases) |
| UnityAds | 4.19.0.1 | Banner, Interstitial, Rewarded | **✓** | **✓** | **✓** | **✓** | [view](UnityAds/CHANGELOG.md) | [link](https://docs.unity.com/ads/en-us/manual/Changelog) |
| Verve | 3.9.0.1 | Banner, Interstitial, Rewarded | **✓** | **✗** | **✗** | **✗** | [view](Verve/CHANGELOG.md) | [link](https://github.com/pubnative/pubnative-hybid-ios-sdk/releases) |
| YangoAds | 8.3.0.0 | Banner, Interstitial, Rewarded, AppOpen, Native | **✓** | **✓** | **✓** | **✓** | [view](YangoAds/CHANGELOG.md) | [link](https://github.com/yandexmobile/yandex-ads-sdk-ios/blob/master/changelog/YandexMobileAds.md) |
| YsoNetwork | 1.2.1.0 | Banner, Interstitial, Rewarded | **✓** | **✓** | **✗** | **✓** | [view](YsoNetwork/CHANGELOG.md) | [link]() |


> [!NOTE]  
> Adapters marked with 🧪 are currently in closed beta. These adapters are not recommended for use in all applications. For more information, contact your account manager.

#### Columns description
- **SPM** — Indicates whether Swift Package Manager integration is supported.
- **Optimal** — The adapter is included in the Optimal Ads Solutions bundle.
- **Families** — The adapter is included in the Families Ads Solutions bundle.
- **VPN** — The adapter is included in the VPN-compliant Ads Solutions bundle.
- **Changelog** — Link to the adapter changelog.
- **SDK** — Link to the ad framework release notes.

## Adapters in Cocoapods
All adapters for supported networks have the `CASMediation` prefix, and do not require additional frameworks dependencies to be added.  

Adapters in the Podfile have the following format, where `[Name]` and `[Version]` are values from the Adapters table:
```ruby
pod 'CleverAdsSolutions-Base', $casVersion
pod 'CASMediation[Name]', '[Version]'
```

## Adapters in Swift Package Manager
Adapters in Swift Package Manager (SPM) integration are updated only with the CAS version, so you don’t need to track adapter and ad framework versions separately.

Some adapters do not support SPM integration due to limitations of the ad framework. 
We monitor framework updates and will add SPM support once the issues are resolved.

> [!NOTE]  
> If the adapters important to you do not support SPM integration, you will need to integrate all adapters using CocoaPods.


## Adapters versioning
The adapter versioning scheme for versioned adapters is `<third-party SDK version>.<adapter patch version>`. For example, if an ad network releases a new SDK version `1.2.3`, a new adapter version `1.2.3.0` will be released to Bintray after being tested against that new SDK.

If an adapter needs updating outside the lifecycle of a third-party SDK release, the patch version will increase. A bug fix for adapter version `1.2.3.0` will be released in version `1.2.3.1`.

## CAS compatible versions
Each adapter is updated separately from the SDK update and has its own version. However, you have the option to use compatible adapter versions for the CAS version you are using. Just use `CleverAdsSolutions-SDK` and with the CAS version for all adapters added in this way. CocoaPods will select adapter versions that are compatible with the specified CAS version.

Adapters in the Podfile have the following format, where `[Name]` are value from the Adapters table:
```ruby
pod 'CleverAdsSolutions-SDK/[Name]', $casVersion
```
